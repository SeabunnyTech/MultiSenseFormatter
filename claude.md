...
歸零日期當天若無實測資料，**自動退化（fallback）為以篩選後首筆資料日期作為基準**（處理器印出「歸零日期 X 無資料，改以首筆資料日期 Y 為基準」）。

- 常見於設定的歸零日期遠早於儀器建置日的情況（例如配合 InSAR 2018 影像日期歸零，但儀器 2024 年才有資料）。此時 2018 的日期僅為「設定值」，實際基準是各儀器自身首筆資料日，資料等同於「以自身第一筆歸零」。
- 若歸零日期**晚於**該儀器最後一筆資料日期，則整檔跳過（不輸出）。
- `GNSSProcessor` 與 `ZeroingProcessor` 皆採此邏輯；基準值取用時會 `pd.to_numeric(errors='coerce')`，故原始資料含 `#DIV/0!` 等公式錯誤值時會被視為缺值，不會導致整檔失敗。
